...
public class P2ArtifactReader {
     ...
     */
    public static P2ArtifactCoordinates getArtifact(String repositoryBaseDir, String bundle) throws IOException {
        ...
        // repositoryParser.parse(createPath(repositoryBaseDir).toUri().toURL());
        P2Repository p2Repository = null;
        ...
        for (P2Artifact p2Artifact : p2Repository.getArtifacts().getArtifacts()) {
            P2ArtifactCoordinates foundArtifact = new P2ArtifactCoordinates(p2Artifact.getId(), p2Artifact.getVersion(), p2Artifact.getClassifier());
            if (foundArtifact.equals(artifactToFind)) {
                addProperties(foundArtifact, p2Artifact, repositoryBaseDir);
                String bundleFilename = P2ArtifactRuleProcessor.getFilename(p2Repository.getMappings(), foundArtifact);
    ...
    private static void addProperties(P2ArtifactCoordinates foundArtifact, P2Artifact p2Artifact, String baseDir) {
        ...
        if (properties != null) {
            properties.getPropertites().forEach(property -> foundArtifact.addProperty(property.getName(), property.getValue()));
        }
    }
