...
public class MavenArtifactUtils {
    ...
    public static Gav convertArtifactToGav(Artifact artifact) {
        return new Gav(artifact.getGroupId(), StringUtils.defaultString(artifact.getArtifactId()), StringUtils.defaultString(artifact.getVersion()), artifact.getClassifier(), artifact.getType(), null, null, null, false, null, false, null);
    }
    ...
    public static Gav convertPathToGav(RepositoryPath repositoryPath) throws IOException {
     ...
     */
    public static Pair<String, String> getDirectoryGA(RepositoryPath directoryPath) throws IOException {
        String path = RepositoryFiles.relativizePath(directoryPath);
        if (path.endsWith("/")) {
            path = StringUtils.substringBeforeLast(path, "/");
        }
        return Pair.with(StringUtils.substringBeforeLast(path, "/").replaceAll("/", "."), StringUtils.substringAfterLast(path, "/"));
    }
