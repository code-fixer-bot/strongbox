...
public class CollectionUtils {
    ...
    public static Object getMapValue(String path, Map<String, Object> map) {
        if (path == null || path.trim().isEmpty()) {
            return map;
        }
        Object result = map;
    ...
    private static Stream<Map.Entry<String, Object>> flatten(Map.Entry<String, Object> entry) {
        ...
        if (v instanceof Map) {
            return ((Map<String, Object>) v).entrySet().stream().flatMap(CollectionUtils::flatten).collect(HashMap<String, Object>::new, flattenedEntryConsumer(k), HashMap::putAll).entrySet().stream();
        } else if (v instanceof Collection) {
            AtomicInteger i = new AtomicInteger();
            Function<? super Object, ? extends String> flattenedKeyMapper = o -> k + "[" + i.getAndIncrement() + "]";
            return flatten(((Collection<Object>) v).stream().map(CollectionUtils::flattenObject).collect(Collectors.toMap(flattenedKeyMapper, CollectionUtils::flattenObject)));
        }
